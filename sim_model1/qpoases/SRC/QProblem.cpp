...
returnValue QProblem::hotstart(
    ...
    if (printlevel == PL_HIGH) {
        ...
        return getGlobalMessageHandler()->throwWarning(
...
returnValue QProblem::getDualSolution(real_t* const yOpt) const {
    ...
    if ((getStatus() == QPS_AUXILIARYQPSOLVED) || (getStatus() == QPS_HOMOTOPYQPSOLVED)
 ...
 */
returnValue QProblem::solveInitialQP(
    ...
    if (setupAuxiliaryWorkingSet(&auxiliaryBounds, &auxiliaryConstraints, BT_TRUE)
        != SUCCESSFUL_RETURN)
        return THROWERROR(RET_INIT_FAILED);
    ...
    if (setupAuxiliaryQPbounds(&auxiliaryBounds, &auxiliaryConstraints, BT_TRUE)
        != SUCCESSFUL_RETURN)
        return THROWERROR(RET_INIT_FAILED);
...
returnValue QProblem::obtainAuxiliaryWorkingSet(
    ...
    if (QProblemB::obtainAuxiliaryWorkingSet(xOpt, yOpt, guessedBounds, auxiliaryBounds)
        != SUCCESSFUL_RETURN)
 ...
 */
returnValue QProblem::setupAuxiliaryWorkingSet(
    const Bounds* const auxiliaryBounds,
            ...
            || (auxiliaryConstraints->getStatus(i) == ST_UPPER))
        {
            ...
            if (constraints.getStatus(i) == ST_INACTIVE) {
                ...
                if (addConstraint_checkLI(i) == RET_LINEARLY_INDEPENDENT) {
                    if (addConstraint(i, auxiliaryConstraints->getStatus(i), updateCholesky)
                        != SUCCESSFUL_RETURN)
                        return THROWERROR(RET_SETUP_WORKINGSET_FAILED);
                }
            }
 ...
 */
returnValue QProblem::setupAuxiliaryQPbounds(
    const Bounds* const auxiliaryBounds,
    const Constraints* const auxiliaryConstraints,
 ...
 */
returnValue
QProblem::addConstraint(int number, SubjectToStatus C_status, BooleanType updateCholesky) {
    int i, j, ii;
...
/* setup output preferences */
    ...
    return getGlobalMessageHandler()
        ->throwInfo(RET_LI_RESOLVED, 0, __FUNCTION__, __FILE__, __LINE__, VS_HIDDEN);
}

/*
 *	a d d B o u n d
 */
returnValue QProblem::addBound(int number, SubjectToStatus B_status, BooleanType updateCholesky) {
    int i, j, ii;
 ...
 */
returnValue QProblem::removeConstraint(int number, BooleanType updateCholesky) {
    int i, j, ii, jj;
    ...
    if (number_idx < nAC - 1) {
        ...
        for (j = (nAC - 2 - number_idx); j >= 0; --j) {
            computeGivens(
                T[(nAC - 2 - j) * NVMAX + tcol + 1 + j],
                T[(nAC - 2 - j) * NVMAX + tcol + j],
                T[(nAC - 2 - j) * NVMAX + tcol + 1 + j],
                T[(nAC - 2 - j) * NVMAX + tcol + j],
                c,
                s
 ...
 */
returnValue QProblem::removeBound(int number, BooleanType updateCholesky) {
    int i, j, ii, jj;
 ...
 */
returnValue QProblem::backsolveR(const real_t* const b, BooleanType transposed, real_t* const a) {
 ...
 *	b a c k s o l v e R  (CODE DUPLICATE OF QProblemB CLASS!!!)
 */
returnValue QProblem::backsolveR(
...
returnValue QProblem::hotstart_determineDataShift(
    ...
    QProblemB::hotstart_determineDataShift(
        FX_idx,
        g_new,
        lb_new,
        ub_new,
            ...
            if (nZ > 0) {
                ...
                if ((nAC > 0) && (nFX > 0) && (Delta_bB_isZero == BT_FALSE)) {
                    ...
                    if ((nAC == 0) && (Delta_bB_isZero == BT_TRUE)) {
                        ...
                        if (nAC
                            > 0) /* => Delta_bB_isZero == BT_TRUE, as BT_FALSE would imply nFX>0 */
                        {
                            for (j = 0; j < nFR; ++j) {
                                jj = FR_idx[j];
...
/* print a stepsize warning if stepsize is zero */
        ...
        char messageString[80];
        sprintf(messageString, "Stepsize is %.6e", tau);
        getGlobalMessageHandler()->throwWarning(
            RET_STEPSIZE,
            messageString,
            __FUNCTION__,
            __FILE__,
 ...
 */
BooleanType QProblem::areBoundsConsistent(
    const real_t* const delta_lb,
    const real_t* const delta_ub,
    const real_t* const delta_lbA,
    const real_t* const delta_ubA
 ...
 */
returnValue QProblem::setupQPdata(
    const real_t* const _H,
    const real_t* const _R,
    const real_t* const _g,
    const real_t* const _A,
 ...
 */
returnValue QProblem::printIteration(
    int iteration,
    int BC_idx,
    SubjectToStatus BC_status,
    BooleanType BC_isBound
 ...
 *	p r i n t I t e r a t i o n
 */
returnValue QProblem::printIteration(int iteration, int BC_idx, SubjectToStatus BC_status) {
