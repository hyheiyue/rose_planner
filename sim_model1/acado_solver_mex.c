...
void printMatrix(const char* name, real_t* mat, unsigned nRows, unsigned nCols) {
    unsigned r, c;
    mexPrintf("%s: \n", name);
    for (r = 0; r < nRows; ++r) {
        for (c = 0; c < nCols; ++c)
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    const char* infoNames[6] = { "status",   "cpuTime", "kktValue",
                                 "objValue", "QP_iter", "QP_violation" };
    mxArray* info;
    ...
    if (nlhs != 1)
        mexErrMsgTxt("This function returns one output.");
...
#if (ACADO_HARDCODED_CONSTRAINT_VALUES == 0) \
    && ((ACADO_QP_SOLVER == ACADO_QPOASES) || (ACADO_QP_SOLVER == ACADO_QPOASES3))
    /* Bounds */
...
#elif (ACADO_HARDCODED_CONSTRAINT_VALUES == 0) && (ACADO_QP_SOLVER == ACADO_FORCES) \
    && !ACADO_BLOCK_CONDENSING
    /* Bounds */
    #if (ACADO_QP_NLB > 0)
...
#elif (ACADO_HARDCODED_CONSTRAINT_VALUES == 0) && (ACADO_QP_SOLVER == ACADO_QPDUNES) \
    && !ACADO_BLOCK_CONDENSING
    /* Bounds */
