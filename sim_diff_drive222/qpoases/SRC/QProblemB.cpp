 ...
 *	h o t s t a r t
 */
returnValue QProblemB::hotstart(
 ...
 */
returnValue QProblemB::solveInitialQP(
    ...
    if (guessedBounds != 0) {
        ...
        for (i = 0; i < nV; ++i) {
            if (bounds.getType(i) == ST_EQUALITY) {
                ...
                if (auxiliaryBounds->setupBound(i, guessedBounds->getStatus(i))
                    != SUCCESSFUL_RETURN)
                    return THROWERROR(RET_OBTAINING_WORKINGSET_FAILED);
            }
        }
 ...
 */
returnValue
QProblemB::setupAuxiliaryWorkingSet(const Bounds* const auxiliaryBounds, BooleanType setupAfresh) {
    int i;
    int nV = getNV();
 ...
 */
returnValue
QProblemB::setupAuxiliaryQPsolution(const real_t* const xOpt, const real_t* const yOpt) {
    int i;
    int nV = getNV();
 ...
 *	a d d B o u n d
 */
returnValue QProblemB::addBound(int number, SubjectToStatus B_status, BooleanType updateCholesky) {
    int i, j;
    int nFR = getNFR();
...
returnValue QProblemB::removeBound(int number, BooleanType updateCholesky) {
    int i, ii;
    int nFR = getNFR();
 ...
 */
returnValue QProblemB::backsolveR(const real_t* const b, BooleanType transposed, real_t* const a) {
 ...
 *	b a c k s o l v e R  (CODE DUPLICATED IN QProblem CLASS!!!)
 */
returnValue QProblemB::backsolveR(
 ...
 */
BooleanType
QProblemB::areBoundsConsistent(const real_t* const delta_lb, const real_t* const delta_ub) const {
    int i;
 ...
 */
returnValue QProblemB::setupQPdata(
    const real_t* const _H,
    const real_t* const _R,
    const real_t* const _g,
    const real_t* const _lb,
 ...
 */
returnValue QProblemB::printIteration(int iteration, int BC_idx, SubjectToStatus BC_status) {
