...
returnValue readFromFile(real_t* data, int nrow, int ncol, const char* datafilename) {
    ...
    if ((datafile = fopen(datafilename, "r")) == 0) {
        ...
        return getGlobalMessageHandler()->throwError(
 ...
 */
returnValue
writeIntoFile(const real_t* const data, int n, const char* datafilename, BooleanType append) {
 ...
 */
returnValue
writeIntoFile(const int* const data, int n, const char* datafilename, BooleanType append) {
    int i;
