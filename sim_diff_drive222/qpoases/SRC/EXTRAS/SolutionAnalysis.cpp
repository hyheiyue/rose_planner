...
returnValue
SolutionAnalysis::getVarianceCovariance(QProblem* qp, real_t* g_b_bA_VAR, real_t* Primal_Dual_VAR) {
    ...
    for (run3 = 0; run3 < KKT_DIM; run3++) {
        for (run1 = 0; run1 < NVMAX; run1++) {
            ...
            delta_lb_cov[run1] =
                g_b_bA_VAR[run3 * KKT_DIM + NVMAX + run1]; /*  LINE-WISE LOADING OF THE INPUT */
            delta_ub_cov[run1] =
                g_b_bA_VAR[run3 * KKT_DIM + NVMAX + run1]; /*  VARIANCE-COVARIANCE            */
        }
        for (run1 = 0; run1 < NCMAX; run1++) {
