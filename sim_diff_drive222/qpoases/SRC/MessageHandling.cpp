...
MessageHandling::ReturnValueList returnValueList[] = {
    ...
    { RET_CONSTRAINTS_NOT_SPECIFIED,
      "Problem does comprise constraints! You have to specify new constraints' bounds",
      VS_VISIBLE },
    ...
    { RET_CYCLING_NOT_RESOLVED,
      "Cycling cannot be resolved, QP is probably infeasible",
      VS_VISIBLE },
...
returnValue MessageHandling::throwMessage(
    ...
    if ((returnValueList[keypos].globalVisibilityStatus == VS_VISIBLE)
        && (localVisibilityStatus == VS_VISIBLE))
    {
        if (errorCount > 0) {
...
returnValue MessageHandling::throwMessage(
    ...
    if (linenumber == 0)
        i++;
    if (localVisibilityStatus == VS_VISIBLE)
        i++;
    if (RETstring == 0)
        i++;
    /* END OF DUMMY CODE */
