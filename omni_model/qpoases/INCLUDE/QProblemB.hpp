...
public:
    ...
    returnValue init(
        ...
        int&
            nWSR, /**< Input: Maximum number of working set recalculations when using initial homotopy. \n
    ...
    /** Returns Hessian matrix of the QP (deep copy).
		  *	\return SUCCESSFUL_RETURN */
    inline returnValue
    getH(real_t* const _H /**< Array of appropriate dimension for copying Hessian matrix.*/
    ) const;

    /** Returns gradient vector of the QP (deep copy).
		  *	\return SUCCESSFUL_RETURN */
    inline returnValue
    getG(real_t* const _g /**< Array of appropriate dimension for copying gradient vector.*/
    ) const;

    /** Returns lower bound vector of the QP (deep copy).
		  *	\return SUCCESSFUL_RETURN */
    inline returnValue
    getLB(real_t* const _lb /**< Array of appropriate dimension for copying lower bound vector.*/
    ) const;

    /** Returns single entry of lower bound vector of the QP.
		  *	\return SUCCESSFUL_RETURN \n
					RET_INDEX_OUT_OF_BOUNDS */
    inline returnValue getLB(
        int number, /**< Number of entry to be returned. */
    ...
    /** Returns upper bound vector of the QP (deep copy).
		  *	\return SUCCESSFUL_RETURN */
    inline returnValue
    getUB(real_t* const _ub /**< Array of appropriate dimension for copying upper bound vector.*/
    ) const;

    /** Returns single entry of upper bound vector of the QP.
		  *	\return SUCCESSFUL_RETURN \n
					RET_INDEX_OUT_OF_BOUNDS */
    inline returnValue getUB(
        int number, /**< Number of entry to be returned. */
    ...
    /** Returns the dual solution vector.
		 *	\return SUCCESSFUL_RETURN \n
					RET_QP_NOT_SOLVED */
    returnValue
    getDualSolution(real_t* const yOpt /**< Output: Dual solution vector (if QP has been solved). */
    ) const;
...
protected:
    ...
    returnValue setupAuxiliaryWorkingSet(
        const Bounds* const auxiliaryBounds, /**< Working set for auxiliary QP. */
    ...
    /** Checks if lower/upper bounds remain consistent
		 ...
		 */
    BooleanType areBoundsConsistent(
    ...
    /** Sets Hessian matrix of the QP.
		 *	\return SUCCESSFUL_RETURN */
    inline returnValue
    setH(const real_t* const H_new /**< New Hessian matrix (with correct dimension!). */
    );

    /** Changes gradient vector of the QP.
		 *	\return SUCCESSFUL_RETURN */
    inline returnValue
    setG(const real_t* const g_new /**< New gradient vector (with correct dimension!). */
    );

    /** Changes lower bound vector of the QP.
		 *	\return SUCCESSFUL_RETURN */
    inline returnValue
    setLB(const real_t* const lb_new /**< New lower bound vector (with correct dimension!). */
    );
    ...
    /** Changes upper bound vector of the QP.
		 *	\return SUCCESSFUL_RETURN */
    inline returnValue
    setUB(const real_t* const ub_new /**< New upper bound vector (with correct dimension!). */
    );
...
protected:
    ...
    BooleanType
        hasHessian; /**< Flag indicating whether H contains Hessian or corresponding Cholesky factor R; \sa init. */
    ...
    BooleanType
        hasCholesky; /**< Flag indicating whether Cholesky decomposition has already been setup. */
